...
export const meta = {
  ...
  lastEdited: '2019-12-18T15:50:35.000Z'
}
...
Listed below are the permissions we require from GitHub along with the reasons why.
...
| `Checks`          | Y    | Y     | Allows us to add checks against user source code on push.                    |
| `Contents`        | Y    | Y     | Allows us to fetch and write source code for new project creation templates. |
| `Deployments`     | Y    | Y     | Allows us to synchronize deployment status between GitHub and ZEIT.          |
| `Pull Requests`   | Y    | Y     | Allows us create deployments per each pull request and comment on them.      |
| `Issues`          | Y    | Y     | GitHub considers all Pull Requests to be Issues.                             |
...
| `Commit Statuses` | Y    | Y     | Allows us to syncronize commit status between GitHub and ZEIT.               |
...
<Note>
  ...
  permission scopes. Please reach out to support@zeit.co.
</Note>
