...
export const meta = {
  ...
  lastEdited: '2019-12-17T04:03:00.000Z'
}
...
| `Checks`          | Y    | Y     | Allows us to run checks against user source code on push and.       |
| `Contents`        | Y    | Y     | Allows us to compare file hashes for de-duplication.                |
| `Deployments`     | Y    | Y     | Allows us to synchronize deployment status between GitHub and ZEIT. |
| `Issues`          | Y    | Y     | ???                                                                 |
| `Metadata`        | Y    | N     | Allows us to read basic repository metadata.                        |
| `Pull Requests`   | Y    | Y     | Lets us create deployments per each pull request.                   |
